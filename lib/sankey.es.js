...
var Sankey = (function () {
    ...
    Sankey.prototype.dragStart = function (d) {
        if (!d._drag) {
            d._drag = { x: event.x, x0: d.x0, x1: d.x1, y: event.y, y0: d.y0, y1: d.y1 };
        }
    };
    Sankey.prototype.dragMove = function (d, cx) {
        ...
        select(this)
            ...
            var dx = event.x - d1._drag.x;
            var dy = event.y - d1._drag.y;
    ...
    Sankey.prototype.draw = function () {
        ...
        node.append("rect")
            ...
            .attr("stroke", function (d) { return color(d.fill).darker(1); })
            .append("title")
        ...
        node.attr("cursor", "move")
            ...
            .on("end", function (d) { self.dragEnd(d); select(this).raise(); }));
        ...
        link.on("click", function (d) {
            ...
            var glink = select("#g_" + nodes[1] + "_" + nodes[2]);
            glink.raise();
            var srcnode = select("#g_" + nodes[1]);
            srcnode.raise();
            var tgtnode = select("#g_" + nodes[2]);
            tgtnode.raise();
        });
    ...
    Sankey.prototype.reset = function () { reset.call(this); return this; };
    return Sankey;
}());
