...
var Line = (function () {
    function Line(parentid, id) {
        this._bisectDate = bisector(function (d) { return d[0]; }).left;
        this._boundCharts = [];
    ...
    Line.prototype.data = function (data) {
        ...
        var ex = extent(dom, function (d) { return d[1]; });
        this.domain.y = [0, ex[1] * 1.05];
        return this;
    };
    Line.prototype.draw = function () {
        ...
        g.each(function (d) {
            ...
            grp.append("path")
                ...
                .attr("d", areaLine)
                .attr("class", "areaLine")
                .attr("stroke", function () { return color(d.fill).darker(1); })
                .on("click", function () {
                grp.raise();
                self._topLayer = grp;
            });
    ...
    Line.prototype.moveBookmark = function (axisValue, remote) {
        ...
        this._bookmark.select("text.bookmark.x")
            ...
            .text(this._multiDateFormat(data[0]));
        return this;
    };
    Line.prototype.reset = function () { reset.call(this); return this; };
    Line.prototype._multiDateFormat = function (date) {
        return (second(date) < date ? this.formatMillisecond
            : minute(date) < date ? this.formatSecond
                : hour(date) < date ? this.formatMinute
                    : day(date) < date ? this.formatHour
                        : month(date) < date ? (sunday(date) < date ? this.formatDay : this.formatWeek)
                            : year(date) < date ? this.formatMonth
