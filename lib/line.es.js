...
function formatLocale$2(locale) {
  var group = locale.grouping && locale.thousands ? formatGroup$1(locale.grouping, locale.thousands) : identity$6,
      currency = locale.currency,
      decimal = locale.decimal,
      numerals = locale.numerals ? formatNumerals$1(locale.numerals) : identity$6,
      percent = locale.percent || "%";
...
function nodrag$1(view) {
  var root = view.document.documentElement,
      selection$$1 = select$1(view).on("dragstart.drag", noevent$1, true);
  if ("onselectstart" in root) {
...
function formatLocale$3(locale) {
  var group = locale.grouping && locale.thousands ? formatGroup$2(locale.grouping, locale.thousands) : identity$7,
      currency = locale.currency,
      decimal = locale.decimal,
      numerals = locale.numerals ? formatNumerals$2(locale.numerals) : identity$7,
      percent = locale.percent || "%";
...
function formatDate(date) {
    return (second$2(date) < date ? timeFormat$2(".%L")
        : minute$2(date) < date ? timeFormat$2(":%S")
            : hour$2(date) < date ? timeFormat$2("%I:%M")
                : day$2(date) < date ? timeFormat$2("%I %p")
                    : month$2(date) < date
                        ? (sunday$2(date) < date
                            ? timeFormat$2("%a %d")
                            : timeFormat$2("%b %d"))
                        : year$2(date) < date ? timeFormat$2("%b")
                            : timeFormat$2("%b %Y"))(date);
}
function isDate(d) {
    var pd = Date.parse(d);
    var rd = isNaN(d);
...
var Line = (function () {
    function Line(parentid, id) {
        this._bisectXAxis = bisector(function (d) { return d[0]; }).left;
        this._boundCharts = [];
        this._curveFunction = curveLinear;
        this._showArea = true;
        ...
        this.scale = { c: ordinal(schemeCategory10), x: undefined, y: undefined };
    ...
    Line.prototype.curve = function (show) {
        ...
        this._curveFunction = show ? curveBasis : curveLinear;
        return this;
    };
    Line.prototype.data = function (data) {
        ...
        this.domain.x = extent(dom, function (d) { return d[0]; });
        this.domain.y = extent(dom, function (d) { return d[1]; });
        return this;
    };
    Line.prototype.draw = function () {
        ...
        g.each(function (d) {
            ...
            grp.append("path")
                ...
                .attr("d", areaLine)
                .attr("class", "areaLine")
                .attr("stroke", function () { return self._showArea ? color(d.fill).darker(1) : d.fill; })
                .on("click", function () {
                grp.raise();
                self._topLayer = grp;
            });
            if (self._showPoints) {
                grp.selectAll("circle")
                    ...
                    .append("circle")
                    .attr("class", "area")
                    .attr("fill", function () { return self._showArea ? color(d.fill).darker(1) : d.fill; })
            ...
            var lb = grp.append("text")
                ...
                .call(drag()
                .on("start", function () { select(this).raise().classed("active", true); })
                .on("drag", function () { select(this).attr("x", event.x).attr("y", event.y); })
                .on("end", function () { select(this).classed("active", false); }));
    ...
    Line.prototype.moveBookmark = function (axisValue, remote) {
        ...
        var msgy = isDate(data[1]) ? formatDate(data[1]) : data[1];
        ...
        var msgx = isDate(data[0]) ? formatDate(data[0]) : data[0];
