...
var Axis = (function () {
    ...
    Axis.prototype.drawYAxis = function () {
        ...
        canvas.append("g")
            ...
            .tickFormat(self.formatNumber));
        return this;
    };
    Axis.prototype.formatDate = function (date) {
        return (second(date) < date ? timeFormat(".%L")
            : minute(date) < date ? timeFormat(":%S")
                : hour(date) < date ? timeFormat("%I:%M")
                    : day(date) < date ? timeFormat("%I %p")
                        : month(date) < date
                            ? (sunday(date) < date
                                ? timeFormat("%a %d")
                                : timeFormat("%b %d"))
                            : year(date) < date ? timeFormat("%b")
                                : timeFormat("%b %Y"))(date);
    };
    Axis.prototype.formatNumber = function (n) {
        return (n > 999 || n % 1 !== 0
            ? format("~s")
            : format(""))(n);
    };
